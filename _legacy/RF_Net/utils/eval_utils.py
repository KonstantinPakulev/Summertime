...
from legacy.RF_Net.utils.math_utils import distance_matrix_vector, pairwise_distances, ptCltoCr
...
def eval_model(model, parsed_batch, DES_THRSH, COO_THRSH):
    ...
    rep = rep_score(kp1w, kp2, visible, COO_THRSH)

    return TPNN, PNN, TPNNT, PNNT, TPNNDR, PNNDR, rep


def rep_score(kp1w, kp2, visible, COO_THRESH):
    coo_dist_matrix = pairwise_distances(
        kp1w[:, 1:3].float(), kp2[:, 1:3].float()
    )

    nn_value, _ = coo_dist_matrix.min(dim=-1)

    correct_match_label = nn_value.le(COO_THRESH) * visible
