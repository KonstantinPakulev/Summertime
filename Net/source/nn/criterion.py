...
class HardQuadTripletLoss(HardTripletLoss):
    ...
    def __init__(self, grid_size, margin, num_neigh, num_neg, loss_lambda):
        super().__init__(grid_size, margin, num_neigh, num_neg, loss_lambda)
...
class HardQuadTripletSOSRLoss(nn.Module):
    ...
    def __init__(self, grid_size, margin, num_neigh, num_neg, sos_neg, loss_lambda):
        super().__init__()
    ...
    def forward(self, kp1, w_kp1, kp1_desc, desc2):
        ...
        # Calculate SOS
        # TODO. Should be replaced in future as well as the whole negative sampling procedure
        radius = 3 * self.grid_size * np.sqrt(2)
